				 ...
				 int flags)
{
	...
	pr_warn("[WARN] Uncaught enqueue, CONTEXT: p=[pid=%d, cpu=%d, state=%d, on_cpu=%d, on_rq=%d, ipanema=[current_state=%s]]; rq[%d]=%p; flags=%d\n",
...
static void update_curr_ipanema(struct rq *rq)
{
	...
	schedstat_set(curr->stats.exec_max,
		      max(delta_exec, (u64)curr->stats.exec_max));
				 ...
				 int flags)
{
	...
	pr_warn("[WARN] Uncaught dequeue, CONTEXT: p=[pid=%d, cpu=%d, state=%d, on_cpu=%d, on_rq=%d, ipanema=[current_state=%s]]; rq[%d]=%p; flags=%d\n",
		p->pid, task_cpu(p), READ_ONCE(p->__state), p->on_cpu, p->on_rq,
		ipanema_state_to_str(ipanema_task_state(p)),
		rq->cpu, rq, flags);
						  ...
						  struct rq_flags *rf)
{
	...
	/* 
	 * Ca me semble inutile maintenant car le seul endroit où 
	 * sched_class->pick_next_task(rq) est appelé c'est dans __pick_next_task() dans core.c,
	 * juste avant il y a un appel à put_prev_task_balance(prev) qui appel 
	 * sched_class->put_prev_task(prev).
	 * Donc je devrai pouvoir changer la signature sans causer de soucis si on appelle
	 * pas put_prev_task ici. A verifier encore.
	 */
	if (result != prev) {
		if (prev)
...
{	
	return list_first_entry(head, struct task_struct, ipanema.node_list);
